...
export default function Workspace() {
  const [activeTool, setActiveTool] = useState<'select' | 'direct-select' | 'rect' | 'pen' | 'text' | 'type-on-path' | 'artboard' | 'shape-builder' | 'pencil' | 'brush' | 'eraser'>('select');
  ...
  const {
    ...
    updateShape, updateShapes, deleteShape, moveShape,
    groupShapes, ungroupShapes,
    copy, paste, duplicate,
    addToHistory,
    assets, addAsset, removeAsset, addOrUpdateImage,
  ...
  useEffect(() => {
    const handleBeforeUnload = (e: BeforeUnloadEvent) => {
      // If there is history or shapes, warn the user
      if (history.past.length > 0 || shapes.length > 0) {
        e.preventDefault();
        e.returnValue = '';
      }
    };
  ...
  }, [history.past.length, shapes.length]);
            ...
            if (selectedIds.length > 0) {
                ...
                selectedIds.forEach(id => {
                    ...
                    if (shape) {
                        ...
                        updates[id] = { x: shape.x + dx, y: shape.y + dy };
                    }
                });
